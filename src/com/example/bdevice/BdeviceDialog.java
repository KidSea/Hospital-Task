...
public class BdeviceDialog extends JDialog implements ActionListener{
		// 定义控件
		JPanel jp;
	...
	public BdeviceDialog(JFrame owner, String title, boolean modal) {
	...
	public void actionPerformed(ActionEvent e) {
		...
		if (e.getSource() == jb1) {
			...
			// 跳出连接对话框
			new BdConnectDig(Mjframe, "连接界面", true, bd, rowNum);
			...
			//删除数据库中的一条记录 
			String bdId = (String) bd.getValueAt(rowNum, 0);
			//创建一个SQL语句
			String sql = "delete from bdadd where 序列=?";
			String [] paras ={bdId};
			BdeviceModel temp = new BdeviceModel();
			temp.updBdevice(sql, paras);
			
			//构建新的数据模型类，并更新
		    bd = new BdeviceModel();
		    String []paras2 = {"1"};
		    bd.queryBd("select * from bdadd where 1=?",paras2);
		    //更新
		    jt.setModel(bd);
		} else if (e.getSource() == jb3) {
			SelectDialog dialog = new SelectDialog(Mjframe, "选择端口", true);
			port = dialog.port;
			type = 1;
